...
import { PoemCategory } from "@/src/constants/poem-categories";
...
export default function PoemsScreen() {
    ...
    const filteredPoems = useMemo(() => {
        ...
        return poems.filter((poem) => {
            return (
                normalizeSearchValue(poem.title.on).includes(query) ||
                normalizeSearchValue(poem.title.ru).includes(query) ||
                (poem.title.en
                    ? normalizeSearchValue(poem.title.en).includes(query)
                    : false)
            );
        });
    }, [poems, searchQuery]);
    const sections = useMemo(() => {
        const grouped = filteredPoems.reduce<
            Partial<Record<PoemCategory, typeof filteredPoems>>
        >((acc, poem) => {
        ...
        return (
            Object.entries(grouped) as [PoemCategory, typeof filteredPoems][]
        ).map(([category, data]) => ({
            category,
            data,
        }));
    }, [filteredPoems]);
    const continueTitle = lastOpenedPoem
        ? uiLanguage === "ru"
            ? lastOpenedPoem.titleRu
            : lastOpenedPoem.titleEn
        : "";
    const gradientColors: [string, string, string] =
        colors.background === "#0B0B0C"
            ? ["rgba(0,0,0,0.95)", "rgba(0,0,0,0.6)", "rgba(0,0,0,0)"]
            : [
                ...
                >
                            ...
                            <Text
                                style={[
                                    styles.heroEyebrow,
                                    {
                                        color: colors.accent,
                                        fontSize:
                                            typography.labelSmall * fontScale,
                                    },
                                ]}
                            >
                                {uiLanguage === "ru"
                    ...
                    {!!lastOpenedPoem && (
                        <PressableCard
                            ...
                            onPress={() =>
                                router.push(`/poem/${lastOpenedPoem.slug}`)
                            }
                        >
                            <Text
                                style={[
                            ...
                            <Text
                                style={[
                                    styles.continueOriginalTitle,
                                    {
                                        color: colors.textPrimary,
                                        fontSize:
                                            typography.titleSmall * fontScale,
                                    },
                                ]}
                            >
                    ...
                    <SectionList
                        ...
                        ListEmptyComponent={
                            ...
                            >
                                ...
                                >
                                    {uiLanguage === "ru"
                                        ? "Попробуй ввести другое название или очистить поиск."
